...
public class UserGroupService implements IUserGroupService {
	...
	public PageResult<UserGroup> queryUserGroups(Website website, String gname,
		 ...
		 int count = this.userGroupDao.getUserGroupCount(website, gname, userGroupType, propVal, fromModifiedTime, toModifiedTime);
	...
	public PageResult<GrouppedUser> queryExcludeUsers(long gid, Limit limit) {
		...
		if(count > 0) {
			List<Long> uidList = this.dynamicUserGroupService.queryExcludedUids(gid, limit.offset, limit.maxRows);
	...
	@Override
	public DataIterator<Long> queryGrouppedUids(long gid, int limit) throws GroupUserQueryException {
		...
		return new UserGroupBatchIterator(this, gid, manualCount, propertyCount, limit);
	}
	
	@Override
	public PageResult<GrouppedUser> queryGrouppedUsers(long gid, Limit limit) throws GroupUserQueryException {
		long manualCount = this.dynamicUserGroupService.getUserCount(gid);
		long propertyCount = this.propertiesGrouppedUserService.getUserCount(gid);
		long excludeCount = this.dynamicUserGroupService.getExcludeUserCount(gid);
		propertyCount = propertyCount - excludeCount;
		...
		List<GrouppedUser> grouppedUserList = this.queryGrouppedUsers(gid, manualCount, propertyCount, limit.offset, limit.maxRows);
		
		this.assembleUserToGrouppedUser(grouppedUserList);

		PageResult<GrouppedUser> result = new PageResult<GrouppedUser>(grouppedUserList, limit, (int)count);
		return result;
	}
	...
	private void assembleUserToGrouppedUser(List<GrouppedUser> grouppedUserList) {
		Map<Long, GrouppedUser> grouppedUsers = new HashMap<Long, GrouppedUser>(grouppedUserList.size());
		for(GrouppedUser grouppedUser : grouppedUserList) {
			grouppedUsers.put(grouppedUser.getUid(), grouppedUser);
		}
		
		List<User> users = this.userService.getUsersByIds(new ArrayList<Long>(grouppedUsers.keySet()));
		for(User user : users) {
	...
	@Override
	public List<GrouppedUser> queryGrouppedUsers(long gid, long manualCount, long propertyCount, int offset, int limit) throws GroupUserQueryException{
		...
		if(offset < manualCount) {
			//需要从手动的地方查询
			List<Long> manualUids = this.dynamicUserGroupService.queryGrouppedUids(gid, offset, (int)(endIndex - offset + 1));
		...
		if(leftLimit > 0) {
			...
			List<Long> propertyUids = this.propertiesGrouppedUserService.queryGrouppedUids(gid, leftOffset, (int)(endIndex - leftOffset + 1));
	    ...
	    List<GrouppedUser> rtnList = new ArrayList<GrouppedUser>(manualList.size() + propertyList.size());
	...
	public PageResult<GrouppedUser> queryManualUsers(long gid, Limit limit) {
		...
		List<GrouppedUser> list = Collections.emptyList();
		if(count > 0) {
			List<Long> uidList = this.dynamicUserGroupService.queryGrouppedUids(gid, limit.offset, limit.maxRows);
			List<GrouppedUser> grouppedUserList = this.convertUidToGrouppedUser(uidList, true);
			this.assembleUserToGrouppedUser(grouppedUserList);
			list = grouppedUserList;
		}
