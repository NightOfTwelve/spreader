...
public class TxtToRedisRead<V> {
	...
	public TxtToRedisRead(String redisPrefix, String txtDir, RedisTemplate<String, Object> redisTemplate, Converter<String, V> valueConverter) {
		...
		RedisWrite<String, V> innerRedisWrites = new RedisWrite<String, V>(redisTemplate);
		writes = new RawWrites<String,V>(new WriteWrap<String, V, String, V>(innerRedisWrites, converters.pToCp, null));
		TxtReads<V> txtReads = new TxtReads<V>(valueConverter);
		reads = new CommonCachedReads.SameValue<String, URL, String, V>(converters.pToOp, converters.pToCp, txtReads, writes);
	}
}
