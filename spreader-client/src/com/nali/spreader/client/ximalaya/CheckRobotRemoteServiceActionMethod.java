	...
	public Object execute(Map<String, Object> params, Map<String, Object> userContext, Long uid) {
		...
		try {
			...
			if (list == null || list.size() == 0) {
				sendMail(mail, "[警告][营销系统-主动关注]调用robotRemoteService接口报警", "http://robot.ximalaya.com/robot/hessian/robotRemoteService queryUser接口返回数据为空");
			} else {
				if (sendSuccessEmail) {
					sendMail(mail, "[正常][营销系统-主动关注]调用robotRemoteService接口正常", "http://robot.ximalaya.com/robot/hessian/robotRemoteService queryUser接口调用正常  list.size:" + list.size());
				}
				return true;
			}
		} catch (AuthenticationException e) {
			sendMail(mail, "[错误][营销系统-主动关注]调用robotRemoteService接口错误", "http://robot.ximalaya.com/robot/hessian/robotRemoteService queryUser接口错误 异常：AuthenticationException\n" + e);
			logger.error(e, e);
			throw new RuntimeException(e);
		} catch (Exception e) {
			sendMail(mail, "[错误][营销系统-主动关注]调用robotRemoteService接口错误", "http://robot.ximalaya.com/robot/hessian/robotRemoteService queryUser接口错误 异常：Exception\n" + e);
			logger.error(e, e);
			throw new RuntimeException(e);
		}
		return false;
